...
function findMaxRange(ranges) {
    const numArr = ranges.map((arr) => arr.match(/-?[0-9]+(\.[0-9]+)?/g));
    const maxArr = numArr.map((set) => Math.abs(set[1] - set[0]));
    
    let finalArray = []
    maxArr.forEach((item, i) => {
        if(Math.max(...maxArr) === item) finalArray.push(ranges[i])
    })
